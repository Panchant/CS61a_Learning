...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        exit -- The Place reaFd by exiting this Place (may be None).
        """
        self.name = name
        self.exit = exit
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            cur_ant = place.ant
            if cur_ant.can_contain(self):
                cur_ant.store_ant(self)
            elif self.can_contain(cur_ant):
                place.ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur_range = 0
        while cur_place.is_hive == False:
            if cur_range >= self.lower_bound and cur_range <= self.upper_bound and len(cur_place.bees) > 0:
                return random_bee(cur_place.bees) # REPLACE THIS LINE
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            bee_list = self.place.bees
            if bee_list:
                target_bee = random_bee(bee_list)
    ...
    def action(self, gamestate):
        ...
        while cur_place:
            if isinstance(cur_place.ant, ContainerAnt):
                ...
                if cur_place.ant.ant_contained:
